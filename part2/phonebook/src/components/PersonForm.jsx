  ...
  const onSubmitHandle = (event) => {
    ...
    if (persons.map((persons) => persons.name).includes(newName)) {
      if (
        window.confirm(
          newName +
            " is already added to the phonebook, do you want to update the phone number?"
        )
      ) {
        let findId = persons.find((persons) => persons.name === newName);
        phoneService
          .update(findId.id, newAdd)
          .then((returnedObject) => {
            setPersons(
              persons.map((persons) =>
                persons.id !== findId.id ? persons : returnedObject
              )
            );
          })
          .then(() => {
          ...
          .catch(() => {
            setErrorMessage("Update of " + newName + " failed");
            setTimeout(() => {
              setErrorMessage(null);
            }, 5000);
          });
      ...
      phoneService
        ...
        .then(() => {
          setErrorMessage("Created " + newAdd.name);
          setTimeout(() => {
            setErrorMessage(null);
          }, 5000);
        })
      ...
      <table>
        <tbody>
          ...
          <tr>
            <td>number: </td>
            <td>
              <input onChange={addNumber} value={newNumber} />
            </td>
          </tr>
        </tbody>
      </table>
